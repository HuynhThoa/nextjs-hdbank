...
aws ecr get-login-password --region ap-southeast-1 --profile old-lab | docker login --username AWS --password-stdin 695660539618.dkr.ecr.ap-southeast-1.amazonaws.com > NUL
...
aws ecs update-service --service ThuHoService --region ap-southeast-1 --profile old-lab --desired-count 0 --cluster ThuHo > NUL

timeout 30 > NUL

aws ecs update-service --service ThuHoService --region ap-southeast-1 --profile old-lab --desired-count 2 --cluster ThuHo > NUL
