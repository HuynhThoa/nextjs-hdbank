...
import { PARTNER } from "commons/constants";
import { getPartner } from "commons/helpers";
interface Props {
  open: boolean;
  onClose: () => void;
      ...
      <Box p={2}>
        <Grid container justifyContent="space-between" spacing={1}>
          <Grid item xs={7} className={classes.textContent}>
            <Box>
              <img
                src="/epass/asset/images/HDBanklogo.png"
                style={{ width: "100px" }}
              />
            </Box>
            <Box>
              Quý khách đã thực hiện giao dịch nạp tiền giao thông. Quý khách
              đánh giá giao dịch như thế nào?
            </Box>
          </Grid>
          <Grid item xs={5} className={classes.imageRatingWrapper}>
            <img
              style={{ width: "90%" }}
              src="/epass/asset/images/Rating.svg"
              alt="rating-icon"
            />
          </Grid>
        </Grid>
